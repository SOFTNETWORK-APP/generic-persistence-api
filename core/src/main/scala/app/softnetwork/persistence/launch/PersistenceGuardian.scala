...
trait PersistenceGuardian extends ClusterDomainEventHandler { _: SchemaProvider =>
  ...
  def setup(): Behavior[ClusterDomainEvent] = {
    Behaviors.setup[ClusterDomainEvent] { context =>
      ...
      Behaviors.receiveMessagePartial { case event: ClusterDomainEvent =>
...
object PersistenceGuardian {
  implicit def entity2PersistentEntity[C <: Command, S <: State, E <: Event, R <: CommandResult](
    entity: EntityBehavior[C, S, E, R]
  ): PersistentEntity[C, S, E, R] = {
    PersistentEntity(entity, Some(entity.role))
  }
}
