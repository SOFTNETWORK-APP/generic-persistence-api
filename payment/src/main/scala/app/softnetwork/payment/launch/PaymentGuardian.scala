...
trait PaymentGuardian extends NotificationGuardian with SessionGuardian {_: SchemaProvider =>
  ...
  override def entities: ActorSystem[_] =>  Seq[PersistentEntity[_, _, _, _]] = sys =>
    schedulerEntities(sys) ++ notificationEntities(sys) ++ sessionEntities(sys) ++ paymentEntities(sys)
  ...
  override def eventProcessorStreams: ActorSystem[_] => Seq[EventProcessorStream[_]] = sys =>
    schedulerEventProcessorStreams(sys) ++ notificationEventProcessorStreams(sys) ++ paymentEventProcessorStreams(sys)
