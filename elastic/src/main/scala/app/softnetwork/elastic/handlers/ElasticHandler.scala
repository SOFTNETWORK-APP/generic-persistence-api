...
trait ElasticDao[T <: Timestamped] { _: ElasticHandler[T] =>
  ...
  def create(document: T)(implicit
    system: ActorSystem[_],
    m: Manifest[T]
  ): Future[Either[ElasticResult, DocumentCreated]] = {
  ...
  def update(document: T)(implicit
    system: ActorSystem[_],
    m: Manifest[T]
  ): Future[Either[ElasticResult, DocumentUpdated]] = {
    implicit val ec: ExecutionContextExecutor = system.executionContext
  ...
  def upsert(id: String, data: Map[String, Any])(implicit
    system: ActorSystem[_]
  ): Future[Either[ElasticResult, DocumentUpserted]] = {
    implicit val ec: ExecutionContextExecutor = system.executionContext
  ...
  def delete(id: String)(implicit
    system: ActorSystem[_],
    m: Manifest[T]
  ): Future[Either[ElasticResult, DocumentDeleted]] = {
    implicit val ec: ExecutionContextExecutor = system.executionContext
  ...
  def load(id: String)(implicit
    system: ActorSystem[_]
  ): Future[Either[ElasticResult, DocumentLoaded[_ <: Timestamped]]] = {
    implicit val ec: ExecutionContextExecutor = system.executionContext
  ...
  def search(query: SQLQuery)(implicit
    system: ActorSystem[_]
  ): Future[Either[ElasticResult, DocumentsFound[_ <: Timestamped]]] = {
    implicit val ec: ExecutionContextExecutor = system.executionContext
  ...
  def count(query: SQLQuery)(implicit
    system: ActorSystem[_]
  ): Future[Either[ElasticResult, ElasticResult with CountResult]] = {
    implicit val ec: ExecutionContextExecutor = system.executionContext
  ...
  def bulkUpdate(documents: List[Map[String, Any]])(implicit
    system: ActorSystem[_]
  ): Future[Either[ElasticResult, message.DocumentsBulkUpdated.type]] = {
    implicit val ec: ExecutionContextExecutor = system.executionContext
  ...
  def bulkDelete(documents: List[Map[String, Any]])(implicit
    system: ActorSystem[_]
  ): Future[Either[ElasticResult, message.DocumentsBulkDeleted.type]] = {
    implicit val ec: ExecutionContextExecutor = system.executionContext
  ...
  def flush(
    index: Option[String]
  )(implicit system: ActorSystem[_]): Future[Either[ElasticResult, message.IndexFlushed.type]] = {
    implicit val ec: ExecutionContextExecutor = system.executionContext
  ...
  def refresh(
    index: Option[String]
  )(implicit system: ActorSystem[_]): Future[Either[ElasticResult, message.IndexRefreshed.type]] = {
    implicit val ec: ExecutionContextExecutor = system.executionContext
