...
object Queries {
  ...
  val leftPredicate = "select * from Table where ((identifier1 = 1) and (identifier2 > 2)) or (identifier3 = 3)"
  val rightPredicate = "select * from Table where (identifier1 = 1) and ((identifier2 > 2) or (identifier3 = 3))"
  val predicates = "select * from Table where ((identifier1 = 1) and (identifier2 > 2)) or ((identifier3 = 3) and (identifier4 = 4))"
  val nestedPredicate = "select * from Table where (identifier1 = 1) and nested((nested.identifier2 > 2) or (nested.identifier3 = 3))"
  val nestedCriteria = "select * from Table where (identifier1 = 1) and nested(nested.identifier3 = 3)"
  val childPredicate = "select * from Table where (identifier1 = 1) and child((child.identifier2 > 2) or (child.identifier3 = 3))"
  val childCriteria = "select * from Table where (identifier1 = 1) and child(child.identifier3 = 3)"
  val parentPredicate = "select * from Table where (identifier1 = 1) and parent((parent.identifier2 > 2) or (parent.identifier3 = 3))"
  val parentCriteria = "select * from Table where (identifier1 = 1) and parent(parent.identifier3 = 3)"
  ...
  val inNumericalExpressionWithDoubleValues = "select * from Table where identifier in (1.0,2.1,3.4)"
  val notInLiteralExpression = "select * from Table where identifier not in (\"val1\",\"val2\",\"val3\")"
  val notInNumericalExpressionWithIntValues = "select * from Table where identifier not in (1,2,3)"
  val notInNumericalExpressionWithDoubleValues = "select * from Table where identifier not in (1.0,2.1,3.4)"
  val nestedWithBetween = "select * from Table where nested((ciblage.Archivage_CreationDate between \"now-3M/M\" and \"now\") and (ciblage.statutComportement = 1))"
  ...
  val countNested = "select count(email.value) as email from crmgp where profile.postalCode in (\"75001\",\"75002\")"
  val isNull = "select * from Table where identifier is null"
  val isNotNull = "select * from Table where identifier is not null"
  val geoDistanceCriteria = "select * from Table where distance(profile.location,(-70.0,40.0)) <= \"5km\""
}
...
class SQLParserSpec extends AnyFlatSpec with Matchers {
  ...
  it should "parse count with nested criteria" in {
    ...
    result.right.get.sql should === ("select count(email.value) as email from crmgp where nested(profile.postalCode in (\"75001\",\"75002\"))")
  }
